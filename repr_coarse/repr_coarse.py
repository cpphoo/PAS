...
def main(args):
    ...
    if args.use_cosine_clf:
        ...
        clf = models.cosine_clf(feature_dim, num_coarse_classes).cuda()
    else:
        if args.remove_last_relu:
            warnings.warn(
                "Using linear classifier with the last relu activation removed!")
        clf = nn.Linear(feature_dim, num_coarse_classes).cuda()
    ############################
...
def train(backbone, clf, optimizer, trainloader, secondary_trainloader, criterion, 
    ...
    for i, (X, y) in enumerate(trainloader):
        ...
        perf = utils.accuracy(logits.data, y_combined[:, -2].data, topk=(1, 5), 
                    compute_mean_class=False)
        log_probability = F.log_softmax(logits, dim=1)
        loss = criterion(log_probability, y_combined[:, -2])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Script to train the Repr-Coarse Representation')
