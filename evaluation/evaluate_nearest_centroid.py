...
def generate_prototypes(reps, labels, num_categories, num_shots, cosine=False):
    '''
        Generate prototypes.
        ...
        num_shots: number of examples used to construct the prototype. if non-positive, then use do all-shot
    ...
    for i in range(num_categories):
        ind = torch.nonzero(labels == i, as_tuple=False).squeeze()
    ...
    for s in args.shots:
        ...
        for i in tqdm(range(num_repetitions)):
            ...
            prototypes_novel, _ = generate_prototypes(
                reps_ref_novel, labels_ref_novel[:, -1], num_novel, s, args.cosine)
