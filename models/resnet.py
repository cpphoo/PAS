# Adapted based on https://pytorch.org/vision/stable/_modules/torchvision/models/resnet.html#resnet18
...
class ResNet(nn.Module):
    ...
    def __init__(self, block_type, layers, zero_init_residual=False,
                 ...
                 remove_last_relu=False, high_res_input=True):
        ...
        # if the resolution of the input is not high, then use 3x3 conv
        if not high_res_input:
        ...
        self.layer1 = self._make_layer(
            block_type, 64, layers[0], norm_layer=norm_layer, 
            remove_last_relu=False)
        self.layer2 = self._make_layer(
            block_type, 128, layers[1], stride=2, norm_layer=norm_layer, 
            remove_last_relu=False)
        self.layer3 = self._make_layer(
            block_type, 256, layers[2], stride=2, norm_layer=norm_layer, 
            remove_last_relu=False)
        self.layer4 = self._make_layer(
            block_type, 512, layers[3], stride=2, norm_layer=norm_layer, 
    ...
    def _make_layer(self, block_type, planes, blocks, stride=1, norm_layer=None, 
                    remove_last_relu=False):
        if block_type == 'basic':
            block = BasicBlock
